$(document).ready(() => {
  ...
  const contentDiv = document.getElementById('curriculum-content-content');
  ...
  if (params.has('subject') && params.has('content') && params.has('uid')) {
    ...
    contentRef.get().then((contentSnapshot) => {
      if (contentSnapshot && contentSnapshot.exists) {
        ...
        iframe.setAttribute(
          'src',
          `https://www.youtube.com/embed/${embedID}?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1`
        );
